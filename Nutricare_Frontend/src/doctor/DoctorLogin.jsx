...
import { doctorLoginThunk } from "../store/slices/doctorAuthSlice";

export default function DoctorLogin() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { loading, error } = useSelector((s) => s.doctorAuth);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const result = await dispatch(doctorLoginThunk({ email, password }));
    ...
    <div className="min-h-[calc(100vh-140px)] flex items-center justify-center px-6">
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-100 shadow-xl p-7">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Sign in to review diet plans and add notes.
        </p>
